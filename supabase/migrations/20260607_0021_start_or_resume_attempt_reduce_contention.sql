...
-- Fix: el UPDATE incondicional a simulator_versions.has_attempts convertia esa
-- fila en un punto de serializacion. Cada estudiante que inicia un intento del
-- mismo simulador tomaba el lock de la MISMA fila y se procesaban en serie; con
-- una clase completa iniciando a la vez, los ultimos en la cola superaban el
-- statement_timeout ("canceling statement due to statement timeout").
--
-- Solucion: hacer el UPDATE condicional para que solo ocurra la primera vez.
-- Una vez que has_attempts = true, el WHERE no matchea ninguna fila, no se toma
-- lock y desaparece la contencion entre inicios concurrentes.
...
begin
  ...
  -- Condicional: evita reescribir la fila (y tomar su lock) en cada inicio.
  update public.simulator_versions sv
  set has_attempts = true
  where sv.id = v_simulator.published_version_id
    and sv.has_attempts is distinct from true;
